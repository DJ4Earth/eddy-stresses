...
function cpintegrate(chkp, scheme)::Float64
    ...
    @ad_checkpoint scheme for chkp.i = 1:chkp.S.grid.nt
        ...
        if chkp.i in chkp.data_steps
            ...
            chkp.J += sum(abs.(fft(chkp.data[1][:,:,chkp.j])[1:120, 1:120] - fft(temp.u)[1:120, 1:120])) / (120*120) +
                sum(abs.(fft(chkp.data[2][:,:,chkp.j])[1:120, 1:120] - fft(temp.v)[1:120, 1:120])) / (120*120)
