function comp_u_v_eta_t(u, v, eta, params, interp_ops, grad_ops, advec_ops) 
    ...
    # bottom friction
    bfric_u = params.bottom_drag .* ((interp_ops.ITu * (kinetic).^(1/2)) .* u) ./ h_u
    bfric_v = params.bottom_drag .* ((interp_ops.ITv * (kinetic).^(1/2)) .* v) ./ h_v

    # deal with the advection term 
    adv_u, adv_v = comp_advection(q, U, V, advec_ops)

    Mu = params.A_h .* (grad_ops.Lu * grad_ops.Lu) * u
    Mv = params.A_h .* (grad_ops.Lv * grad_ops.Lv) * v 
