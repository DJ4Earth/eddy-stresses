...
mutable struct manystatenlp_Chkp{T, S} <: AbstractNLPModel{T,S}
...
function manystatenlp_Chkp{T}(Ndays,init_t,param_guess,lower_bound,upper_bound) where {T<:AbstractFloat}
    ...
    data = [uhrcg[:,:,(init_t+1):end], vhrcg[:,:,(init_t+1):end], etahrcg[:,:,(init_t+1):end]]

    u0, v0, eta0, _ = ShallowWaters.add_halo(uhrcg[:,:,init_t],vhrcg[:,:,init_t],etahrcg[:,:,init_t],zeros(128,128),Slr)
    ...
    return manystatenlp_Chkp{T, typeof(param_guess)}(meta, Counters(), Slr, initial_cond, data, data_steps, 0.0, 1, 1, 0.0, zeros(128,128), zeros(128,128))
