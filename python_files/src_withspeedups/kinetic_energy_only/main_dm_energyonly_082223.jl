...
function for_optim!(F, G, nu)

    days = 30
     init_states = load_object("/Users/swilliamson/Documents/GitHub/eddy-stresses/data_files/states_nx128_ny128_10year_060523.jld2")

     grid = build_grid(3840e3, 3840e3, 128, 128)
     gyre_params = def_params(grid)
 
     # building discrete operators
     grad_ops = build_derivs(grid)                # discrete gradient operators
     interp_ops = build_interp(grid, grad_ops)    # discrete interpolation operators (travels between grids)
     advec_ops = build_advec(grid)
     ...
     data = load_object("/Users/swilliamson/Documents/GitHub/eddy-stresses/data_files/energy_afterspinup_nxny512_2years_everystep_062823.jld2")
     ...
     snaps = Int(floor(sqrt(states_rhs.T)))
     revolve = Revolve{SWM_pde}(states_rhs.T, snaps; verbose=1, gc=true, write_checkpoints=false)
...
# compute the initial grid of nu values 
grid = build_grid(3840e3, 3840e3, 128, 128)
params = def_params(grid)
