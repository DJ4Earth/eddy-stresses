...
function run_checkpointing(tsteps, snaps)
    # Type precision
    T = Float32
    steps = tsteps     # instead of running Ndays I'm just running some number of steps
    data_steps = 1:tsteps
    ...
    snaps = snaps
    revolve = Revolve{Chkp{T, T}}(tsteps,
        snaps;
        verbose=1,
        gc=true,
        write_checkpoints=false,
        write_checkpoints_filename = "",
        write_checkpoints_period = 10
    )
    ...
    println("Cost with Checkpointing + AD: $Jnew")
