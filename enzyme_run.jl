...
# function compute_loss(param_guess)
...
# function compute_gradient(G, param_guess)
#     # Type precision
#     T = Float32
...
#         J_inner = exp1_checkpointed_integration(chkp_inner, revolve)
...
#     G === nothing || compute_gradient(G, param_guess)
#     F === nothing || return compute_loss(param_guess)
...
#     fg!_closure(F, G, ic) = FG(F, G, ic)
#     obj_fg = Optim.only_fg!(fg!_closure)
#     result = Optim.optimize(obj_fg, copy(param_guess), Optim.LBFGS(), Optim.Options(show_trace=true, iterations=1))
...
#     data_steps = 1:1:S_true.grid.nt

#     snaps = Int(floor(sqrt(S_true.grid.nt)))
#     revolve = Revolve{exp1_Chkp}(S_true.grid.nt,
#         snaps;
#         verbose=1,
#         gc=true,
#         write_checkpoints=false,
...
#     chkp_prim = deepcopy(chkp)
#     J_outer = exp1_checkpointed_integration(chkp_prim, revolve)

#     autodiff(set_runtime_activity(Enzyme.ReverseWithPrimal), exp1_checkpointed_integration,
#         Duplicated(chkp, dchkp),
#         Const(revolve)
#     )
...
# diffs, enzyme_deriv, chkp_prim, dchkp = finitedifference(1,1)
