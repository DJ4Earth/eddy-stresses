...
mutable struct kespec4_Chkp{T1,T2}
...
function kespec4_checkpointed_integration(chkp, scheme)
...
function kespec4_integration(chkp, truncate)
...
function kespec4_compute_loss(Ndays, param_guess, data, data_steps, initial_cond)
    ...
    J = kespec4_integration(chkp, truncate)

    return J

end

function kespec4_compute_gradient(G, param_guess, data, data_steps, Ndays, initial_cond)
...
function kespec4_FG(F, G, param_guess, data, data_steps, Ndays, initial_cond)

    G === nothing || kespec4_compute_gradient(G, param_guess, data, data_steps, Ndays, initial_cond)
    F === nothing || return kespec4_compute_loss(Ndays, param_guess, data, data_steps, initial_cond)

end

function run_kespec4()
    ...
    # these do NOT contain the initial condition, start at 1
    ucg = load_object("./spinup_files/coarsegrainedu_30days_dailysaves_071525.jld2")
    vcg = load_object("./spinup_files/coarsegrainedv_30days_dailysaves_071525.jld2")
    ...
    fg!_closure(F, G, param_guess) = kespec4_FG(F, G, param_guess, data, data_steps, Ndays, initial_cond)
