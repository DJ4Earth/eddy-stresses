...
function build_derivs(grid)
    ...
    GTx = (diagm(1 => ones(NT-1)) - I)[index1, :] ./ dx 
    GTy = (diagm(nx => ones(Nv)) - I)[1:end-nx, :] ./ dy
    ...
    Guy1 = diagm(diag1)[:, index3][:, nx:end]
    ...
    Gqy1 = diagm(diag1)[:,index3][:, nx:end]
    ...
    Gvx1 = (diagm(Nq, Nv + (ny - 1), -(nx+1) => diag2))[:, index2]
    Gvx2 = diagm(Nq, Nv + (ny - 1), -(nx+1) => (-diag2[end:-1:1]))[:, index22] 
    Gvx = (Gvx2 + Gvx1) ./ dx

    # x-derivative from q-grid to v-grid
    diag2[1:(nx+1):end] .= 1.0
    Gqx1 = (diagm(Nq, Nv + (ny - 1), -(nx+1) => diag2))[:, index2] 
    Gqx2 = diagm(Nq, Nv + (ny - 1), -(nx+1) => -diag2[end:-1:1])[:, index22]   
    Gqx = -(Gqx1 + Gqx2)' ./ dx 
    ...
    return GTx, GTy, Gux, Guy, Gvx, Gvy, Gqy, Gqx, Lu, Lv, LT, Lq
...
function build_interp(grid, grad_ops)
    ...
    diag1 = ones(Nv) ./ 4
    Ivu1 = diagm(Nv+nx, Nv, diag1)
    Ivu2 = diagm(Nv+nx, Nv, 1 => diag1[1:end-1])
    Ivu3 = diagm(Nv+nx, Nv, (-nx + 1) => diag1)
    Ivu4 = diagm(Nv+nx, Nv, -nx => diag1)
    ...
    IqT1 = diagm(NT + ny - 1, Nq, diag2)
    IqT2 = diagm(NT + ny - 1, Nq, 1 => diag2)
    IqT3 = diagm(NT + ny - 1, Nq, (nx + 1) => diag2)
    IqT4 = diagm(NT + ny - 1, Nq, (nx + 2) => diag2)
    ...
    Iqu1 = diagm(Nq - nx - 1, Nq, diag3)
    Iqu2 = diagm(Nq - nx - 1, Nq, (nx + 1) => diag3)
    Iqu = (Iqu1 + Iqu2)[index3, :]

    # u-grid to q-grid 
    Iuq = copy(Iqu)'
    Iuq[2:nx, 1:nx-1] .= diagm(ones(nx-1))
    Iuq[end-nx+1:end-1, end-nx+2:end] .= diagm(ones(nx-1))

    # q-grid to v-grid 
    diag4 = ones(Nv + ny - 2) ./ 2
    index4 = 1:Nv+ny-2 
    index4 = filter(x -> x ∉ [(nx + 1)*j for j in 1:(Nv+ny-2)], index4)
    Iqv1 = diagm(Nv + ny - 2, Nq, (nx+1) => diag4)
    Iqv2 = diagm(Nv + ny - 2, Nq, (nx+2) => diag4)
    ...
    return Ivu, Iuv, IqT, IuT, IvT, ITu, ITv, Iqu, Iuq, Iqv, Ivq, ITq
...
function build_advec(grid)
    ...
    diag1 = ones(Nq) ./ 24 
    two_diag1 = 2 .* diag1
    AL11 = diagm(Nq + nx, Nq, two_diag1)
    AL12 = diagm(Nq + nx, Nq, 1 => diag1[1:end-1]) 
    AL13 = diagm(Nq + nx, Nq, (nx+1) => diag1[1:end-6])
    AL14 = diagm(Nq + nx, Nq, (nx+2) => two_diag1[1:end-7])
    ...
    AL21 = diagm(Nq + nx, Nq, diag1)
    AL22 = diagm(Nq + nx, Nq, 1 => two_diag1[1:end-1]) 
    AL23 = diagm(Nq + nx, Nq, (nx+1) => two_diag1[1:end-6])
    AL24 = diagm(Nq + nx, Nq, (nx+2) => diag1[1:end-7])
    ...
    ALeur1 = diagm(NT + ny - 1, Nq, diag2) 
    ALeur2 = diagm(NT + ny - 1, Nq, 1 => diag2)
    ALeur3 = diagm(NT + ny - 1, Nq, nx+1 => -diag2)
    ALeur4 = diagm(NT + ny - 1, Nq, nx+2 => -diag2)
    ...
    ALeul1 = diagm(NT + ny - 1, Nq, -diag2)
    ALeul2 = diagm(NT + ny - 1, Nq, 1 => -diag2)
    ALeul3 = diagm(NT + ny - 1, Nq, nx+1 => diag2)
    ALeul4 = diagm(NT + ny - 1, Nq, nx+2 => diag2)
    ALeul = (ALeul1 + ALeul2 + ALeul3 + ALeul4)[index3,:]

    ones1 = ones(Nu) 
    ones1[1:nx-1:end] .= 0.0 
    Seur = diagm(Nu, Nu, 1 => ones1[2:end])
    Seul = copy(Seur)'
    ...
    Sau = diagm(Nu + ny, Nv, 1 => ones2[1:Nv-1])[index4, :]
    Scu = diagm(Nu + ny, Nv, ones2[1:Nv])[index4,:]
    Sbu = diagm(Nu + ny, Nv, -(nx - 1) => ones2[1:Nv-1])[index4, :]
    Sdu = diagm(Nu + ny, Nv, -nx => ones2[1:Nv - 1])[index4, :]
    ...
    diag3 = ones(Nq) ./ 24
    ALpvu1 = diagm(Nq, Nq, -diag3)
    ALpvu2 = diagm(Nq, Nq, 1 => diag3[1:end-1])
    ALpvu3 = diagm(Nq, Nq, nx+1 => -diag3[1:end-nx-1])
    ALpvu4 = diagm(Nq, Nq, nx+2 => diag3[1:end-nx-2])
    ...
    ALpvd1 = diagm(Nq, Nq, diag3)
    ALpvd2 = diagm(Nq, Nq, 1 => -diag3[1:end-1])
    ALpvd3 = diagm(Nq, Nq, nx+1 => diag3[1:end-nx-1])
    ALpvd4 = diagm(Nq, Nq, nx+2 => -diag3[1:end-nx-2])
    ALpvd = (ALpvd1 + ALpvd2 + ALpvd3 + ALpvd4)[index6, :]

    ones3 = ones(Nv)
    Spvu = diagm(Nv, Nv, nx => ones3[1:Nv - nx])
    Spvd = copy(Spvu)'

    ones4 = ones(Nv)
    index7 = 1:Nv+nx
    index7 = filter(x -> x ∉ index7[nx:nx:end], index7)
    Sav = diagm(Nv+nx, Nv, -nx => ones4)[index7, :]
    Sbv = diagm(Nv+nx, Nv, -nx + 1 => ones4)[index7, :]
    Scv = diagm(Nv+nx, Nv, ones4)[index7,:]
    Sdv = diagm(Nv+nx, Nv, 1 => ones4[1:end-1])[index7, :]

    return AL1, AL2, index_av, index_bv, index_cv, index_dv, ALeur, ALeul, Seul, Seur, Sau, Sbu, Scu, Sdu, ALpvu, ALpvd, Spvu, Spvd, Sav, Sbv, Scv, Sdv
